...
export const Route = createFileRoute("/members/interviews/$interviewId")({
  component: RouteComponent,
  loader: async ({ params: { interviewId } }) => {
    return getMemberInterviewResult(Number(interviewId));
  },
  pendingComponent: LoadingFullScreen
});

function RouteComponent() {
  const interview = useLoaderData({ from: "/members/interviews/$interviewId" });
  const { interviewId } = useParams({
    from: "/members/interviews/$interviewId"
  });
        ...
        <div className="mt-12">
          <div className="bg-gradient-to-r from-geek-blue-1 to-geek-blue-2 rounded-2xl p-6 border border-geek-blue-3">
            ...
            <div className="space-y-2 text-sm text-geek-blue-7">
              ...
              <p>
                • 개인정보는 포함되지 않으며, 면접 연습 개선을 위한 참고 자료로
                활용됩니다.
              </p>
            </div>
          </div>
