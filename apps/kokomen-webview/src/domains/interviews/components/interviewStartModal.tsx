    ...
    try {
      const handleMessage = (event: MessageEvent): void => {
        const { type, data } = JSON.parse(
          event.data
        ) as WebviewMessage<boolean>;
        if (type === "checkSpeechRecognitionSupported") {
          setIsSpeechRecognitionSupported(data ?? false);
        }
      };
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2">
              ...
              <ul className="text-xs text-blue-800 space-y-1">
                ...
                <li>• 디바이스의 마이크 및 음성인식 권한을 허용해주세요</li>
                <li>• 면접 답변은 크고 명확하게 답변해주세요</li>
              </ul>
            </div>
          </div>
        </div>
